...
The only exclusion is inline functions with [reified type parameters](05-inline-functions.md#reified-type-parameters),
...
For generic functions, using [reified type parameters](05-inline-functions.md#reified-type-parameters) makes casts
like `arg as T` checked, unless `arg`'s type has *its own* type arguments that are erased.

An unchecked cast warning can be suppressed by [annotating](https://kotlinlang.org/docs/annotations.html) the statement or the
...
>**On the JVM**: [array types](06-arrays.md) (`Array<Foo>`) retain information about the erased type of
