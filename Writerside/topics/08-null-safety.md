...
preventing an NPE (`NullPointerException`).
...
* Data inconsistency during initialization, such as when:
  ...
  * A [superclass constructor calling an open member](06-inheritance.md#derived-class-initialization-order) whose implementation
    in the derived class uses an uninitialized state.
* Java interoperation:
  * Attempts to access a member of a `null` reference of a [platform type](https://kotlinlang.org/docs/java-interop.html#null-safety-and-platform-types).
...
> This typically happens with [`lateinit` properties](13-properties.md#late-initialized-properties-and-variables).
>
{style="tip"}
...
```kotlin
    ...
    b = null // Compiles without errors
```
...
```kotlin
    ...
    val l = b.length // Compilation Error: Only safe (?.) or non-null asserted (!!.) calls are allowed on a nullable receiver of type String?
```
...
In the example above, the compiler performs a [smart cast](07-typecasts.md#smart-casts) to change the type from nullable `String?` to non-nullable `String`. It also tracks the information about
...
the [smart cast prerequisites](07-typecasts.md#smart-cast-prerequisites).
...
You can use the `?.` operator with both [`var` and `val` variables](https://kotlinlang.org/docs/basic-syntax.html#variables) in Kotlin:
...
In the example above, if one of the receivers in the safe call chain is `null`, the assignment is skipped, 
and the expression on the right is not evaluated at all. 

For example, if either `person` or `person.department` is `null`, the function is not called. 
...
and the code executes normally. 

However, if the value is `null`, the `!!` operator forces it to be treated as non-nullable, which results in an **NPE**.
...
```kotlin
    ...
    val l = b!!.length // Exception in thread "main" java.lang.NullPointerException
...
You can use extension functions with a [nullable receiver type](03-extensions.md#nullable-receivers),
...
In the example above, even though `person` is `null`, the `.toString()` function safely returns the string `"null"`. 
This can be helpful for debugging and logging.

If you expect the `.toString()` function to return a nullable string (either a string representation or `null`), 
...
[`let` function](14-scope-functions.md#let).
...
The regular Kotlin operator for [type casts](07-typecasts.md#unsafe-cast-operator) is the `as` operator. However, regular casts can result in an exception
