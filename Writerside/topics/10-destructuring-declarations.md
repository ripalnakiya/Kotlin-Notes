...
Variables `a` and `b` get the values returned by `component1()` and `component2()` 
...
So you can freely use destructuring declarations in `for`-loops with 
maps (as well as collections of data class instances or similar).
...
If a lambda has a parameter of the `Pair` type 
...
If a component of the destructured parameter is unused, 
you can replace it with the underscore to avoid inventing its name:
