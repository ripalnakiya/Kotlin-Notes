...
If you don't have these sorts of restrictions, use [collections](https://kotlinlang.org/docs/collections-overview.html) instead.
...
For more information about collections, see [Collections overview](https://kotlinlang.org/docs/collections-overview.html).
## Create arrays
...
Arrays are always mutable. To access and modify elements in an array, use the [indexed access operator](06-operator-overloading.md#indexed-access-operator)`[]`:
...
In Kotlin, you can pass a variable number of arguments to a function via the [`vararg`](01-functions.md#variable-number-of-arguments-varargs)
...
For more information, see [Variable number of arguments (varargs)](01-functions.md#variable-number-of-arguments-varargs).
...
> Don't use equality (`==`) and inequality (`!=`) [operators](09-equality.md#structural-equality) to compare the contents
...
> The `.sum()` function can only be used with arrays of [numeric data types](02-numbers.md), such as `Int`.
>
{style="note"}
...
If you work with different APIs where some use arrays and some use collections, then you can convert your arrays to [collections](https://kotlinlang.org/docs/collections-overview.html)
and vice versa.
...
This example uses the [infix notation](01-functions.md#infix-notation) to call the [`to`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/to.html) function to create tuples of `Pair`:
