...
What do we do if we need them to be invoked **sequentially** &mdash; first `doSomethingUsefulOne` _and then_
...
while `async` returns a [Deferred] &mdash; a light-weight non-blocking future
...
could otherwise continue doing other operations. 
